                                        ...
                                        <div class="row">
                                            ...
                                            <div class="form-group col-md-6">
                                                <select v-model="search.skpd" class="form-control">
                                                    ...
                                                    <option v-for="v in this.skpd_data" :value="v.id" :key="v.id">{{ v.nama_skpd }}</option>
                                                </select>
                                            </div>
                                        </div>
                                        <div class="row">
                    ...
                    <div class="card-body">
                        ...
                        <transition name="fade">
                            <table class="table table-hover table-striped table-bordered" v-if="showTable === true">
                                <thead>
                                    <tr>
                                        <th style="width:20%; text-align:center;">Irban</th>
                                        <th style="width:40%; text-align:center;">Perangkat Daerah</th>
                                        <th style="width:10%; text-align:center;">Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr v-for="v in irbanskpd" :key="v.id">
                        ...
                        <transition name="fade"><v-delete :element="'irbanskpd_delete_modal'" :id="id" @delete="deleteData"></v-delete></transition>
                        <transition name="fade">
...
export default {
    ...
    props: ['irban_data','skpd_data','api','route','access'],
    methods: {
        toggle() {
        ...
        toggleModal(id) {
            $("#irbanskpd_delete_modal").modal('show');
            this.id = id;
        },
        fetchData() {
            this.isLoading = true;
        ...
        deleteData(id) {
            ...
            .then(response => {
                if(response.status === 'ok') {
                    ...
                    $('#irbanskpd_delete_modal').modal('hide');
                    this.fetchData();
