<template>
    <div style="margin-top:25px;">
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="pull-left">
                    <a v-if="(approval_tab.lock === 0 && access.write === 1) && (dinasbop.status === 0 && access.write === 1)" :href="route + '/sekretaris/create?dinasbop=' + dinasbop.id" class="btn btn-success mb-2 mr-2"><i class="fa fa-plus"></i> Tambah Data</a>
                    <span v-if="dinasbopsekretaris.length !== 0 && access.approval === 1 && dinasbop.status === 0">
                            ...
                            <tr v-for="v in dinasbopsekretaris" :key="v.id">
                                ...
                                <td style="text-align: center; vertical-align:middle;">
                                    <div style="text-align: center;">
                                        <a v-if="(approval_tab.lock === 0 && access.update === 1) && (dinasbop.status === 0 && access.update === 1)" :href="route + '/sekretaris/edit?dinasbop='+ dinasbop.id +'&id=' + v.id" class="btn btn-sm btn-warning mr-sm-1">
...
export default {
    ...
    methods: {
        ...
        deleteData(id) {
            service.deleteData(this.api + '/sekretaris?nip='+this.usernip+'&dinasbop='+this.dinasbop.id+'&id='+id)
            .then(response => {
        ...
        createRevision(callback) {
            service.putData(this.api + '/approval?nip='+this.usernip+'&act=revision&type='+callback.role+'&tab=sekretaris&id=' + this.dinasbop.id, {catatan: callback.catatan})
            .then(response => {
                if(response.status === 'ok') {
        ...
        createApproval(role) {
            service.putData(this.api + '/approval?nip='+this.usernip+'&act=approve&type='+role+'&tab=sekretaris&id=' + this.dinasbop.id)
                .then(response => {
                    if(response.status === 'ok') {
